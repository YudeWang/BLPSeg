...
def UpDate_labels_ori(logit, logit_2nd, main_th, aux_th, main_label, ori_label):
    ...
    main_label[ori_label == 255] = 255

    main_label_2nd = main_label.clone()

    return main_label.detach(), main_label_2nd.detach()

def UpDate_labels_3heads(logit, logit_2nd, logit_3rd,main_th, aux_th, main_label, ori_label):
    logit_prob = F.softmax(logit, dim=1)
    logit_prob_max = torch.max(logit_prob, dim=1)[0]
    logit_2nd_prob = F.softmax(logit_2nd, dim=1)
    logit_2nd_prob_max = torch.max(logit_2nd_prob, dim=1)[0]
    ...
    logit_3rd_prob = F.softmax(logit_3rd, dim=1)
    logit_3rd_prob_max = torch.max(logit_3rd_prob, dim=1)[0]
    logit_3rd_label = torch.argmax(logit_3rd_prob, dim=1)

    main_region = ((logit_prob_max > logit_2nd_prob_max).long()+(logit_prob_max > logit_3rd_prob_max).long())
    main_2nd_region = ((logit_2nd_prob_max > logit_prob_max).long()+(logit_2nd_prob_max > logit_3rd_prob_max).long())
    main_3rd_region = ((logit_3rd_prob_max > logit_prob_max).long()+(logit_3rd_prob_max > logit_2nd_prob_max).long())
    
    main_2nd_region[logit_2nd_prob_max < aux_th] = 0
    main_region[logit_prob_max < main_th] = 0
    main_3rd_region[logit_3rd_prob_max < main_th] = 0

    main_label[main_region ==2] = logit_1st_label[main_region ==2]
    main_label[main_2nd_region == 2] = logit_2nd_label[main_2nd_region == 2]
    main_label[main_3rd_region == 2] = logit_3rd_label[main_3rd_region == 2]
    main_label[ori_label == 255] = 255

    main_label_2nd = main_label.clone()
    main_label_3rd = main_label.clone()

    return main_label.detach(), main_label_2nd.detach(), main_label_3rd.detach()

def UpDate_labels_3heads_aff(logit, logit_2nd, logit_3rd, main_th, aux_th, main_label, aff_pred):
    logit_prob = F.softmax(logit, dim=1)
    logit_prob_max = torch.max(logit_prob, dim=1)[0]
    logit_2nd_prob = F.softmax(logit_2nd, dim=1)
    logit_2nd_prob_max = torch.max(logit_2nd_prob, dim=1)[0]
    ...
    main_reliable_label[main_region == 2] = logit_1st_label[main_region == 2]
    main_reliable_label[main_2nd_region == 2] = logit_2nd_label[main_2nd_region == 2]
    main_reliable_label[main_3rd_region == 2] = logit_3rd_label[main_3rd_region == 2]
    ...
    low_main_region[logit_prob_max < 0.3] = 1
    low_main_region_2nd = torch.zeros_like(main_region).long()
    low_main_region_2nd[logit_2nd_prob_max < 0.3] = 1
    low_main_region_3rd = torch.zeros_like(main_region).long()
    low_main_region_3rd[logit_3rd_prob_max < 0.3] = 1

    low_region=torch.zeros_like(main_region).long()
    low_region[(low_main_region+low_main_region_2nd+low_main_region_3rd) == 3] = 1 #==2

    high_main_region = torch.zeros_like(main_region).long()
    high_main_region[logit_prob_max > main_th] = 1
    high_main_region_2nd = torch.zeros_like(main_region).long()
    high_main_region_2nd[logit_2nd_prob_max > main_th] = 1
    high_main_region_3rd = torch.zeros_like(main_region).long()
    high_main_region_3rd[logit_3rd_prob_max > main_th] = 1

    high_region=torch.zeros_like(main_region).long()
    high_region[(high_main_region+high_main_region_2nd+high_main_region_3rd) > 0.1] = 1
    high_region[main_label==255] = 0
    aff_main_label = main_reliable_label.clone()

    m_b, m_h, m_w = aff_main_label.shape
    ...
    logit_3rd_flatten = logit_3rd.resize(l_b, l_c, l_h*l_w)

    aff_pred_logit = 1*logit_flatten.bmm(nor_aff_pred) + 1*logit_2nd_flatten.bmm(nor_aff_pred)+ 1*logit_3rd_flatten.bmm(nor_aff_pred)

    new_pred_main = 0.5*logit_flatten + 0.5*logit_2nd_flatten + 0.5*logit_3rd_flatten + 0.5*aff_pred_logit
    ...
    return main_label, main_label_2nd, main_label_3rd, aff_main_label
...
def UpDate_labels(logit, logit_2nd, main_th, aux_th, main_label, aff_pred):
    ...
    aff_pred = F.normalize(aff_pred.detach(), p=2, dim=1).flatten(2)
    aff_pred = torch.sigmoid(torch.einsum('nca,ncb->nab', aff_pred, aff_pred))
    
    exp_aff_pred = torch.exp(aff_pred-1)
    #print(exp_aff_pred.shape, aff_high_region.shape)
    exp_aff_pred_sum = torch.sum(exp_aff_pred*aff_high_region, dim=1, keepdim=True) #for 3-d, dim=1 for column
    nor_aff_pred = (exp_aff_pred*aff_high_region) / (exp_aff_pred_sum+1e-5)
    ...
    new_pred_main = 0.5*logit_flatten + 0.5*logit_2nd_flatten + 0.5*aff_pred_logit
    new_pred_main = new_pred_main.resize(l_b, l_c, l_h, l_w)
    new_pred_main = torch.argmax(new_pred_main, dim=1)
    main_label[low_region == 1] = new_pred_main[low_region == 1]
    main_label_2nd = main_label.clone()

    # aff_pred_logit_2nd = 1*logit_2nd_flatten.bmm(nor_aff_pred)
    # new_pred_main_2nd = 0.5 * logit_flatten + 0.5 * logit_2nd_flatten + 0.5 * aff_pred_logit_2nd
    # new_pred_main_2nd = new_pred_main_2nd.resize(l_b, l_c, l_h, l_w)
    # new_pred_main_2nd = torch.argmax(new_pred_main_2nd, dim=1)
    # main_label_2nd[low_region == 1] = new_pred_main_2nd[low_region == 1]
...
def compute_affinity_loss(fts_distance, labels, nclass=21):
    ...
    fts_distance = F.normalize(fts_distance, p=2, dim=1).flatten(2)
    fts_distance = torch.sigmoid(torch.einsum('nca,ncb->nab', fts_distance, fts_distance))
