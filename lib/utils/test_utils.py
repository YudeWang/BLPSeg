...
def single_gpu_test_multibatch(model, dataloader, prepare_func, inference_func, collect_func, save_step_func=None):
	model.eval()
	n_gpus = torch.cuda.device_count()
	#assert n_gpus == 1
	collect_list = []
		...
		with torch.no_grad():
			for i_batch, sample in enumerate(dataloader):
				...
				for img in image_msf:
					...
					H = max(H,h)
					W = max(W,w)
				input_batch = torch.zeros((len(image_msf),3,H,W)).cuda().float()
				imgidx = 0
				for img in image_msf:
					_,_,h,w = img.size()
					input_batch[imgidx,:,:h,:w] = img#.cuda()
					imgidx += 1
				result_batch = inference_func(model, input_batch)
				imgidx = 0
				for img in image_msf:
					_,_,h,w = img.size()
					result = result_batch[imgidx,:,:h,:w].unsqueeze(0)
					result_list.append(result)
					imgidx += 1
